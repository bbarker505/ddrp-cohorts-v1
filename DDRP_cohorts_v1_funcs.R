...
PlotMap <- function(r, d, titl, lgd, outfl) {
  ...
  if (grepl("DDtotal", outfl)) {
    ...
    mnth_ct <- plyr::count(dats$mnth) %>% plyr::rename(c("x" = "mnth"))
    # Finally filter unncessary weeks out of generic color key (some months 
    # have only 4 weeks)
    col_key2 <- suppressWarnings(suppressMessages(
                  semi_join(col_key, dats, by = "mnth") %>% 
                  left_join(., mnth_ct, by = c("mnth")) %>%
                  group_by(mnth) %>% 
                  nest() %>%
                  # top_n takes the first n rows; 
                  # keep colors consistent across dates
                  mutate(samp = map(data, ~top_n(data.frame(cols=.$cols), 
                                                 unique(.$freq)))) %>%
                  unnest(samp))) %>% 
                  arrange(mnth)
    dats2 <- dplyr::select(dats, value, mnth) %>% arrange(mnth, value)
    col_key2 <- cbind(col_key2, dplyr::select(dats2, -mnth))
    ...
    # col_key2 <- semi_join(col_key2, df, by=c("value")) %>%
    #   dplyr::select(value, cols)
